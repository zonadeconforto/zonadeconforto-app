...
export class InvestmentProductServiceImpl implements InvestmentProductService {
  ...
  ): Promise<InvestmentProductEntity> {
    if (dto.minValue < 0) throw new ValidationError("minValue must be >= 0");
    if (dto.maxValue < dto.minValue)
      throw new ValidationError("maxValue must be >= minValue");
    if (dto.profitabilityValue <= 0)
      throw new ValidationError("profitabilityValue must be > 0");

    const needsIndexer =
      dto.profitabilityType === "CDI_POST_FIXED" ||
      dto.profitabilityType === "HYBRID";
    if (needsIndexer && !dto.indexer) {
      throw new ValidationError(
        "indexer is required when profitabilityType is post-fixed or hybrid"
      );
    }
  ...
  async update(
    ...
    if (dto.minValue !== undefined && dto.minValue < 0)
      throw new ValidationError("minValue must be >= 0");
    if (
      dto.maxValue !== undefined &&
      dto.minValue !== undefined &&
      dto.maxValue < dto.minValue
    )
      throw new ValidationError("maxValue must be >= minValue");
    if (dto.profitabilityValue !== undefined && dto.profitabilityValue <= 0)
      throw new ValidationError("profitabilityValue must be > 0");

    const needsIndexer =
      (dto.profitabilityType ?? existing.profitabilityType) ===
        "CDI_POST_FIXED" ||
